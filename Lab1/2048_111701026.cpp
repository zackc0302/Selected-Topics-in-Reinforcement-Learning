...
public:
	...
	// Add
	pattern(pattern&& p) : feature(std::move(p)), isomorphic(std::move(p.isomorphic)) {}
	virtual ~pattern() {}
...
public:
	...
	virtual float update(const board& b, float u) {
		...
		float value = 0;
		for (const auto& iso : isomorphic) {
			size_t idx = indexof(iso, b);
			operator[](idx) += adjust;
			value += operator[](idx);
		}
		return value;
	...
	void dump(const board& b, std::ostream& out = info) const {
		for (int i = 0; i < iso_last; i++) {
			...
			for (size_t i = 0; i < isomorphic[i].size(); i++) {
				out << std::hex << ((idx >> (4 * i)) & 0x0f);
			}
			out << std::dec << ") = " << operator[](idx) << std::endl;
		}
	}
